...
import { uploadResume, listResumes, deleteResume, parseResume } from '@/lib/career/resumeApi';
import { ResumeListItem } from '@/lib/types';
import ResumeUploadZone from '@/components/career/ResumeUploadZone';
import { showToast } from '@/lib/swal';
import {
  FileText, Trash2, Sparkles, CheckCircle2, Clock, AlertCircle,
  XCircle, Upload, RefreshCw
} from 'lucide-react';

const STATUS_CONFIG = {
  UPLOADED: { label: 'Uploaded', icon: Upload, cls: 'bg-amber-500/10 border-amber-500/20 text-amber-400' },
  PROCESSING: { label: 'Processing…', icon: Clock, cls: 'bg-cyan-500/10 border-cyan-500/20 text-cyan-400' },
  PARSED: { label: 'Parsed ✓', icon: CheckCircle2, cls: 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' },
  FAILED: { label: 'Failed', icon: XCircle, cls: 'bg-red-500/10 border-red-500/20 text-red-400' },
};
...
export default function ResumePage() {
  ...
  const handleParse = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (result.parsed) {
        showToast('success', `Parsed! ${result.parsed.skills?.length ?? 0} skills extracted.`);
      } else {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold text-white font-outfit">Resume Management</h1>
          <p className="text-sm text-slate-400 mt-1">Upload your PDF resume and let AI extract structured data</p>
        </div>
      ...
      <div className="card">
        <h2 className="font-bold text-slate-200 mb-4 font-outfit flex items-center gap-2">
          ...
          <span className="ml-auto text-xs text-slate-500 font-normal">{resumes.length} file{resumes.length !== 1 ? 's' : ''}</span>
        </h2>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-200 truncate">{r.fileName}</p>
                    <p className="text-[10px] text-slate-500">
                      {new Date(r.createdAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}
                    </p>
                  </div>
                  <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border text-[11px] font-semibold flex-shrink-0 ${s.cls}`}>
                    <StatusIcon size={11} />
                    {s.label}
                  </div>
      ...
      <div className="rounded-xl bg-slate-900/40 border border-slate-800 p-4 text-xs text-slate-400 space-y-1">
        <p className="flex items-center gap-2"><AlertCircle size={12} className="text-amber-400 flex-shrink-0" /> <strong className="text-slate-300">Upload:</strong> PDF format only, maximum 10MB</p>
        <p className="flex items-center gap-2"><Sparkles size={12} className="text-violet-400 flex-shrink-0" /> <strong className="text-slate-300">Parse:</strong> AI will extract skills, experience, education from your resume</p>
        <p className="flex items-center gap-2"><CheckCircle2 size={12} className="text-emerald-400 flex-shrink-0" /> <strong className="text-slate-300">Privacy:</strong> Your resume is stored securely and never shared</p>
      </div>
    </div>
  );
}
