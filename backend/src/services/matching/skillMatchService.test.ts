...
describe('SkillMatchService', () => {
  ...
  it('correctly matches EXACT and ALIAS skills and calculates score', async () => {
    const jobSkills = ['React', 'nodejs', 'postgres', 'Docker'];
    const result = await evaluateSkillMatch(mockCandidateSkills, jobSkills);
  ...
  it('handles job with zero skills specified', async () => {
    const result = await evaluateSkillMatch(mockCandidateSkills, []);
    expect(result.skillScore).toBe(100);
    expect(result.matchedSkills).toHaveLength(0);
    expect(result.missingSkills).toHaveLength(0);
  });

  it('handles candidate with zero skills', async () => {
    const result = await evaluateSkillMatch([], ['React', 'TypeScript']);
